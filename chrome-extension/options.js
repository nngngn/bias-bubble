...
function saveOptions(){
	// identify toggles
	var formElements = document.getElementById("bias-bubble-options").elements;
	...
	var biasCheckSettings = {}
	// populate settings
	targets.forEach(function(e){ biasCheckSettings[e['name']] = e['checked'] })
	chrome.storage.sync.set({
		biasCheckSettings: biasCheckSettings
	}, function(){ console.log("bias-bubble settings updated")});
}

// Restore radio buttons using preferences stored in chrome.storage
function restoreOptions(){
	var els = document.getElementById("bias-bubble-options").elements;
	...
	chrome.storage.sync.get('biasCheckSettings', function(results){
		...
		if(!results['biasCheckSettings']){
			...
			keys.forEach(function(key){
				if(results['biasCheckSettings'][key] == undefined){
					// default setting for any site should be true
					settings[key] = true;
					resubmit = true;
				}else{
					settings[key] = results['biasCheckSettings'][key]
				}
			});
		}
...
const setSourceInfo = function(info){
	...
	// set the similar and alt news here
    ...
    if (req.readyState == 4) {
        if (req.status == 200) {
            ...
            // alert("Data: " + req.responseText);
						...
						document.getElementById('similar1-content').children[0].innerHTML = news[0]["title"];
						document.getElementById('similar1-content').children[1].innerHTML = news[0]["description"];

						document.getElementById('similar2-content').children[0].innerHTML = news[1]["title"];
						document.getElementById('similar2-content').children[1].innerHTML = news[1]["description"];


						document.getElementById('similar3-content').children[0].innerHTML = news[2]["title"];
						document.getElementById('similar3-content').children[1].innerHTML = news[2]["description"];
...
// connect to smart contractcanister running locally
		  ...
		  switch(info.bias){
		    ...
		    case 'satire':
		      ...
		      document.querySelector('#bias-title').style.borderRadius = "15px";
		    	...
		    	// not all satire listings have accuracy ratings for obvious reasons
