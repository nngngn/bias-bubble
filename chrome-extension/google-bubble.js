...
// only run if showGoogle hasnt been disabled
...
function runBiasCheck(){
  ...
  function updateHTML(el, sourceHash){
    ...
    // create opinion HTML
    if(opinion != null){
      decalOpSpan.classList.add('bias-bubble-txt');
    ...
    if(sourceData == null){
      ...
      // create acc HTML
      decalAccSpan.classList.add("bias-bubble-txt");
      ...
      decalAccSpan.textContent = " Accuracy: " + sourceData['accuracy'].toUpperCase() + " ";
      // create bias HTML
      decalBiasSpan.classList.add("bias-bubble-txt");
      ...
      decalLink.setAttribute('class', 'bias-bubble-tag');
  ...
  function run(sourceHash){
    ...
    if(tab.length == 0){
      console.log("bias-bubble can't be used");
      return true;
      ...
      // add bias-bubble class to each element to make sure that script isn't run multiple times on the same element
      standard.forEach(function(e){ e.classList.add('bias-bubble') });
      cards.forEach(function(e){ e.classList.add('bias-bubble') });
      vidCards.forEach(function(e){ e.classList.add('bias-bubble') });
      quotCards.forEach(function(e){ e.classList.add('bias-bubble') });
      // convert to array
      ...
      // add bias-bubble class to each element to make sure that script isn't run multiple times on the same element
      vids.forEach(function(e){ e.classList.add('bias-bubble') });
      // convert to array
